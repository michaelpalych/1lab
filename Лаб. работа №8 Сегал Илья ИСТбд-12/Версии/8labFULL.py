...
class BookingApp:
    ...
    def build_gui(self):
        ...
        table_frame.grid(row=1, column=0, columnspan=3, pady=10)
        self.tree = ttk.Treeview(table_frame, columns=("ФИО","Тип номера","Дней"), show='headings', height=10)
        ...
        # Нижняя панель кнопок сегментации
        ...
        tk.Button(frame, text="Сегментация по типу", width=18, command=self.segment_by_room)\
            .grid(row=2, column=2, sticky='w', pady=5, padx=(10,0))
    ...
    def delete_booking(self):
        ...
        if messagebox.askyesno("Удалить?", "Удалить выбранную бронь?"):
            del self.bookings[idx]; self.refresh_tree()

    def refresh_tree(self):
        for r in self.tree.get_children(): self.tree.delete(r)
        for b in self.bookings: self.tree.insert('', 'end', values=(b.guest_name, b.room_type, b.days))
    ...
    def _draw_pie(self, counts, title, label_func, palette):
        ...
        if total == 0: return
        cx, cy = 450, 320; r = min(900, 600)/2 - 100
        ...
        for i, (key, cnt) in enumerate(items):
            ...
            self.canvas.tag_bind(arc, '<Button-1>',
                lambda e, k=key, c=cnt: messagebox.showinfo('Сегмент', f"{label_func(k)}: {c} ({c/total*100:.1f}%)"))
            start += extent
        lx, ly = cx + r + 40, cy - r
        for i, (key, cnt) in enumerate(items):
            y = ly + i * 30
            self.canvas.create_rectangle(lx, y, lx+20, y+15, fill=palette[i % len(palette)], outline='black')
            self.canvas.create_text(lx+25, y+7, text=f"{label_func(key)}: {cnt} ({cnt/total*100:.1f}%)",
