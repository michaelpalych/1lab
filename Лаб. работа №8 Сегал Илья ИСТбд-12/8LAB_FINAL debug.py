...
class Booking:
    ...
    def from_list(data):
        if len(data) != 3:
            raise ValueError(f"ожидалось 3 поля, получили {len(data)}")
        name, room, days = data
        days = int(days)
        if days < 1:
            raise ValueError("поле «дней» должно быть ≥ 1")
...
class BookingApp:
    def __init__(self, root):
        ...
        self.search_var.trace('w', lambda *a: self.search_by_name())
        self.build_gui()

    def build_gui(self):
        f = tk.Frame(self.root)
        f.pack(padx=10, pady=10)
        tk.Button(f, text="Загрузить из файла", command=self.load_bookings)\
          .grid(row=0, column=0, padx=5)
        tk.Button(f, text="Сохранить в файл", command=self.save_to_file)\
          .grid(row=0, column=1, padx=5)
        tk.Button(f, text="Удалить бронь", command=self.delete_booking)\
          .grid(row=0, column=2, padx=5)
        tk.Entry(f, textvariable=self.search_var, width=20)\
          .grid(row=0, column=3, padx=(20,5))
        tk.Button(f, text="Поиск", command=self.search_by_name)\
          .grid(row=0, column=4, padx=5)

        tf = tk.Frame(f)
        tf.grid(row=1, column=0, columnspan=5, pady=10)
        self.tree = ttk.Treeview(tf, columns=("ФИО","Тип номера","Дней"),
        ...
        tk.Button(f, text="Сегментация по сроку", width=18,
                  command=self.segment_by_days)\
          .grid(row=2, column=0, sticky='e', pady=5, padx=(0,10))
        tk.Button(f, text="Сегментация по типу", width=18,
                  command=self.segment_by_room)\
    ...
    def load_bookings(self):
        ...
        good = []
        errors = []  # список кортежей (номер строки, содержимое, текст ошибки)
        with open(path, encoding='utf-8') as f:
            reader = csv.reader(f)
            for nr, row in enumerate(reader, start=1):
                try:
                    good.append(Booking.from_list(row))
        ...
        if errors:
            # создаём лог-файл рядом с исходным
            base, _ = os.path.splitext(path)
            log_path = base + '_errors.log'
            with open(log_path, 'w', encoding='utf-8') as log:
                log.write(f"Лог ошибок загрузки файла {os.path.basename(path)}\n\n")
                for nr, row, err in errors:
                    log.write(f"Строка {nr}: {row} — ошибка: {err}\n")
    ...
    def save_to_file(self):
        ...
        with open(path, 'w', newline='', encoding='utf-8') as f:
            ...
            for b in self.bookings:
                w.writerow((b.guest_name, b.room_type, b.days))
    ...
    def on_double_click(self, event):
        ...
        if ci == 2:
            new = simpledialog.askinteger("Изменить",
                                          "Число дней:",
                                          initialvalue=old,
                                          minvalue=1)
        else:
            new = simpledialog.askstring("Изменить",
                                         "Новое значение:",
                                         initialvalue=old)
        if new is None:
            return
        if ci == 0:
            self.bookings[idx].guest_name = new
        elif ci == 1:
            self.bookings[idx].room_type = new
        else:
            self.bookings[idx].days = int(new)
    ...
    def _draw_pie(self, counts, title, label_func, palette):
        ...
        for i, (key, cnt) in enumerate(items):
            ...
            self.canvas.tag_bind(arc, '<Button-1>',
                lambda e, k=key, c=cnt: messagebox.showinfo(
                    'Сегмент', f"{label_func(k)}: {c} ({c/total*100:.1f}%)"))
            start += extent
        lx, ly = cx + r + 40, cy - r
        for i, (key, cnt) in enumerate(items):
            y = ly + i*30
